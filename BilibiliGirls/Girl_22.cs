...
namespace BilibiliGirls
{
    internal class Girl_22 : IGirl
    {
        ...
        public Girl_22(ImageList imgList, XmlNode xmlNode)
        {
            #region 初始化图像
            for (int i = 0; i < 5; i++)
            {
                Image img = imgList.Images["22-w" + i.ToString()];
                if (img != null)
                {
                    _standingImageList.Add(img);
                }
            }
            _sayingImage = imgList.Images["22-say"];
            _movingImage = imgList.Images["22-drag"];
            #endregion
            #region 初始化声音
